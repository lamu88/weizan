    ...
    public function doMobileIndex() {
        ...
        if($op=='wemedia'){
            ...
            if($cid==0){
                ...
                $condition .= " AND  ccate = '{$cid}' OR pcate = '{$cid}' ";
        ...
        $condition2.= "where weid=$weid ";
        if($cid==0){
        }else{
            $condition2 .= " AND  ccate='{$cid}' OR pcate = '{$cid}' ";
        }
        $result = pdo_fetchall("SELECT * FROM ".tablename('fineness_article').$condition2."ORDER BY displayorder DESC");
