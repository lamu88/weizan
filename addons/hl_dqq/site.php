...
class Hl_dqqModuleSite extends WeModuleSite {
	...
	public function doWebAwardlist() {
		...
		if (!empty($list)) {
			$sql = "SELECT COUNT(*) FROM " . tablename('dqq_winner')." AS a JOIN " . tablename('mc_mapping_fans')." AS f on f.openid = a.from_user
					JOIN " . tablename('mc_members') . " b ON b.uid = f.uid WHERE a.rid = '$id' $where";
