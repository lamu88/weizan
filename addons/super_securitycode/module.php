...
class Super_securitycodeModule extends WeModule {
	public function fieldsFormDisplay($rid = 0) {
		...
		if($rid==0){
			...
			$reply = pdo_fetch("SELECT * FROM ".tablename('super_securitycode_reply')." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
		}
	...
	public function fieldsFormSubmit($rid) {
		...
		if (empty($id)) {
			...
			pdo_update('super_securitycode_reply', $insert, array('id' => $id));
		}
	}
